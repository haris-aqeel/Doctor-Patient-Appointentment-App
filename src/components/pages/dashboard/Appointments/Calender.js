		...
		const getTableData = async () => {
			try {
				...
				const response = await axios.get(
					`https://edoctor-portal.herokuapp.com/api/v1/appointments/?filter=${formatDate}`,
				);
					...
					<div className="filter-appoitment-container">
						{isLoading ? (
							...
							<div className="table-responsive">
								{filterData.length === 0 ? (
									<h6 className="text-info pt-5">No Appointment Item</h6>
								) : (
									<table className="table text-center">
										...
										<tbody>
											{filterData.map((item, i) => (
												<FilterItem item={item} indx={i + 1} key={item._id} />
											))}
										</tbody>
									</table>
								)}
